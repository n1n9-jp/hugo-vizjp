...
3Dツリーマップ（3D Treemaps）は、**従来のツリーマップを三次元空間に拡張し、面積に加えて高さ（Z軸）で追加の数量的変数を表現する可視化手法**です。「押し出し型（extrusion-based）」とも呼ばれ、各ノードが立体的なブロックとして描画されます。面積でカテゴリーの大きさを、高さで別の数値変数（売上、成長率など）を同時に表現でき、階層データの探索的分析に活用されています。
