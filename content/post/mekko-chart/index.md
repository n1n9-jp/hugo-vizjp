...
メッコ・チャート（Mekko Chart）は、**棒の幅と高さの両方にデータ値をエンコードする二次元の棒グラフ**です。名称は、フィンランドのテキスタイルブランド「Marimekko」の幾何学的な布地パターンに形状が似ていることに由来します。棒の幅は一方の変数（例：市場規模やカテゴリの全体量）を、高さ方向の区画はもう一方の変数（例：各セグメントの構成比率）を表します。
