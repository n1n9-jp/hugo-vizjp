...
## ADS-Bデータ配布サイトの時刻を示す変数と形式

ADS-Bのデータでは、時刻は主にUnixタイムスタンプ（エポック秒またはミリ秒）で記録されることが多く、UTC基準です。一部のデータセットではHH:MM:SS.ss形式も見られますが、共通のデータセット（OpenSky、ADS-B Exchangeなど）ではUnix形式が標準的です。

| サイト          | 時刻を示す変数                          | 形式例                                  |
|-----------------|-----------------------------------------|-----------------------------------------|
| OpenSky Network | - time (Unixエポック秒) | Unixタイムスタンプ (秒, e.g., 1700352000) |
| ADS-B Exchange  | - timestamp (Unixエポックミリ秒または秒) | Unixタイムスタンプ (ミリ秒, e.g., 1700352000780) または秒 |
| Flightradar24   | - timestamp (Unixエポック秒) | Unixタイムスタンプ (秒) |
| ADSB.lol        | - timestamp (Unixエポック秒) | Unixタイムスタンプ (秒, e.g., 1700352000) |
| adsb.fi         | - timestamp (Unixエポックミリ秒) | Unixタイムスタンプ (ミリ秒) |
