...
擬似連続カルトグラム（Pseudo-Continuous Cartogram）は、**地理的な地域を円や四角形、六角形などの幾何図形に置き換え、その面積をデータ変数に比例させて表現する地図手法**です。ドーリング・カルトグラム（Dorling Cartogram）やデマーズ・カルトグラム（Demers Cartogram）とも呼ばれ、元の地域の形状を完全に捨象する代わりに、データの定量的な比較を容易にします。図形は元の地理的位置関係をおおまかに保つように配置されます。
