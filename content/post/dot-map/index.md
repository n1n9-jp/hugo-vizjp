...
ドット・マップ（Dot Map）は、**個々の観測データの位置を地図上にドット（点）で忠実にプロットする地図表現手法**です。1つのドットが1件の事象や1つの観測対象を表し、データの地理的分布を直感的に把握することができます。地震の発生地点や店舗の所在地など、位置情報そのものを可視化する場面で広く利用されています。

<!--more-->

## 別名

- ドット分布マップ（Dot Distribution Map）
